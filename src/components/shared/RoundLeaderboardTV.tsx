...
export default function RoundLeaderboardTV({ 
  players, 
  points, 
  roundPoints,
  roundName
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ width: '100%', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <motion.div layout style={{ display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1, overflowY: 'auto', paddingRight: '1rem', width: '100%' }}>
        {rankedPlayers.map((p: any) => {
          ...
          return (
            <motion.div 
              ...
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: bgGradient, padding: '1.5rem', borderRadius: '1rem', border: `2px solid ${borderColor}`, boxShadow: `0 4px 20px ${borderColor.replace('0.6', '0.15')}` }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flex: 1, minWidth: 0, paddingRight: '1rem' }}>
                <div style={{ 
                  fontSize: '2.5rem', 
                  fontWeight: 'bold', 
                ...
                <span style={{ fontSize: '2.5rem', fontWeight: 'bold', textShadow: '0 2px 4px rgba(0,0,0,0.5)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</span>
              </div>
              <AnimatedScore previousScore={previousScore} roundPoints={rp} targetScore={targetScore} />
            </motion.div>
          );
        })}
      </motion.div>
